...
from helper_scripts.series_diagnostics import compute_intermittency, compute_seasonal_diagnostics, compute_mase
...
def process_single_meter(
    ...
    try:
        ...
        # ===================================================================
        # PER-SENSOR DIAGNOSTICS (no thresholds applied — raw numbers only)
        #   ADI / CV2 / F_S : a-priori, on the clean second training Diff
        #   MASE            : skill vs naive on the prediction segment,
        #                     injected anomalies excluded
        # ===================================================================

        second_diff = df_second["Diff"].values.astype(float)
        ...
        result["n_periods_second"] = inter["n_periods"]
        result["n_nonzero_second"] = inter["n_nonzero"]

        sd = compute_seasonal_diagnostics(second_diff, daily_period_steps, weekly_period_steps)
        result["f_s_daily_second"]        = sd["f_s_daily"]
        result["f_s_weekly_second"]       = sd["f_s_weekly"]
        result["resid_cv2_second"]        = sd["resid_cv2"]
        result["resid_cv2_robust_second"] = sd["resid_cv2_robust"]
        
        # Share of synthetic (gap-filled) rows per segment.
        # A synthetic row has hodnota == NaN — that's exactly the resampling fill.
        # (Anomaly injection alters Diff, not hodnota, so this stays clean.)
        ...
        # MASE numerator = forecast error on NON-anomalous prediction points,
        # so the score reflects baseline tracking, not failure to predict spikes.
